...
namespace FfxivKoreanPatch.FFXIVPatchGenerator
{
    internal sealed class FontPatchGenerator
    {
        ...
        private static readonly uint[] PartyListProtectedPuaGlyphSeeds = new uint[]
        {
            0xE031u,
            0xE037u,
            0xE0B1u, 0xE0B2u, 0xE0B3u, 0xE0B4u,
            0xE0B5u, 0xE0B6u, 0xE0B7u, 0xE0B8u,
        ...
        private const string HighScaleLobbyFallbackSourceFontPath = "common/font/AXIS_18_lobby.fdt";
        ...
        {
            new LobbyLargeLabelVisualScaleSpec("common/font/TrumpGothic_23_lobby.fdt", "common/font/AXIS_18.fdt", 1.00d, 0.80d),
            new LobbyLargeLabelVisualScaleSpec("common/font/TrumpGothic_34_lobby.fdt", "common/font/TrumpGothic_34.fdt", 1.02d, 0.80d)
        };
        ...
        public void Build()
        {
            ...
            LobbyHangulCodepointSets lobbyHangulCodepoints = _options.FontOnly
                ? null
                : CreateLobbyHangulCodepointSets(outputDir, globalSqpack);
        ...
        private static bool LobbyAllocationFits(
            FontGlyphRepairContext glyphRepair,
            LobbyHangulGlyphAllocation allocation,
            int width,
            int height)
        {
            if (string.IsNullOrEmpty(allocation.TexturePath))
            {
                return false;
            }

            int clearWidth = Math.Max(width, allocation.Cell.ClearWidth);
            int clearHeight = Math.Max(height, allocation.Cell.ClearHeight);
            return PatchFitsAllocatedFontTexture(
                glyphRepair,
            ...
            out int normalized)
        {
            ...
            uint[] selectedLobbyHangulCodepoints = SelectLobbyHangulCodepointsForFont(path, lobbyHangulCodepoints);
            int lobbyHangulFixes = ApplyLobbyHangulAllocatedGlyphs(
                ...
                texturePatches,
                lobbyHangulAllocationCache,
                selectedLobbyHangulCodepoints,
                SelectLobbyVisualScaleCodepointsForFont(path, lobbyHangulCodepoints));

            int lobbyCleanAsciiFixes = ApplyHighScaleLobbyCleanAsciiGlyphs(path, ref fdt, glyphRepair, globalArchive, texturePatches);
            int lobbyCleanAsciiKerningFixes = lobbyCleanAsciiFixes > 0
                ? ApplyHighScaleLobbyCleanAsciiKerning(path, ref fdt)
                : 0;
            ...
            if (preChanged == 0 &&
                ...
                lobbyHangulFixes == 0 &&
                lobbyCleanAsciiFixes == 0 &&
                lobbyCleanAsciiKerningFixes == 0 &&
                partyShapeFixes == 0 &&
                cleanAsciiFixes == 0 &&
            ...
            if (lobbyCleanAsciiFixes > 0)
            {
                Console.WriteLine("  Queued high-scale lobby clean ASCII glyph cells: {0} ({1})", lobbyCleanAsciiFixes, path);
            }

            if (lobbyCleanAsciiKerningFixes > 0)
            {
                Console.WriteLine("  Added high-scale lobby clean ASCII kerning pairs: {0} ({1})", lobbyCleanAsciiKerningFixes, path);
            }
        ...
        private static Dictionary<uint, byte[]> ReadGlyphEntriesByUtf8Value(byte[] fdt)
        {
            ...
            for (int i = 0; i < glyphCount; i++)
            {
                ...
                uint codepoint;
                if (TryDecodeFdtGlyphCodepoint(entry, 0, out codepoint))
                {
                    key = PackFdtUtf8Value(codepoint);
                    Endian.WriteUInt32LE(entry, 0, key);
                    ushort shiftJis;
                    if (TryEncodeShiftJisValue(codepoint, out shiftJis))
                    {
                        Endian.WriteUInt16LE(entry, 4, shiftJis);
                    }
                }
                ...
                if (key != utf8Value && !entries.ContainsKey(utf8Value))
                {
            ...
            uint[] visualScaleCodepoints)
        {
            ...
            string baseSourceFdtPath = ResolveLobbyHangulSourceFdtPath(normalizedPath);
            string visualSourceFdtPath = hasVisualScaleSpec
                ? visualScaleSpec.SourceFontPath
                : baseSourceFdtPath;
            string[] lobbyTextureCandidates = GetLobbyHangulTextureCandidates(normalizedPath);
            ...
            FontGlyphRepairContext activeGlyphRepair = transactionalFontPatch
                ? glyphRepair.Clone()
                : glyphRepair;
            LobbyHangulGlyphAllocationCache activeLobbyHangulAllocationCache =
                transactionalFontPatch && lobbyHangulAllocationCache != null
                    ? lobbyHangulAllocationCache.Clone()
                    : lobbyHangulAllocationCache;
            Dictionary<string, List<FontTexturePatch>> activeTexturePatches = transactionalFontPatch
                ? new Dictionary<string, List<FontTexturePatch>>(StringComparer.OrdinalIgnoreCase)
                : texturePatches;

            byte[] baseSourceFdt = TryLoadTtmpStandardPayload(payloadsByPath, mpdStream, baseSourceFdtPath);
            if (baseSourceFdt == null)
            {
                AddLimitedWarning("Lobby Hangul source font missing: " + baseSourceFdtPath + " -> " + normalizedPath);
                return 0;
            }

            Dictionary<uint, byte[]> baseSourceEntries = ReadGlyphEntriesByUtf8Value(baseSourceFdt);
            if (baseSourceEntries.Count == 0)
            {
                return 0;
            }

            byte[] visualSourceFdt = baseSourceFdt;
            Dictionary<uint, byte[]> visualSourceEntries = baseSourceEntries;
            if (!string.Equals(visualSourceFdtPath, baseSourceFdtPath, StringComparison.OrdinalIgnoreCase))
            {
                visualSourceFdt = TryLoadTtmpStandardPayload(payloadsByPath, mpdStream, visualSourceFdtPath);
                if (visualSourceFdt == null)
                {
                    AddLimitedWarning("Lobby visual source font missing: " + visualSourceFdtPath + " -> " + normalizedPath);
                    return 0;
                }

                visualSourceEntries = ReadGlyphEntriesByUtf8Value(visualSourceFdt);
                if (visualSourceEntries.Count == 0)
                {
                    return 0;
                }
            }
            ...
            bool highScaleFallbackSourceLoaded = false;
            Dictionary<uint, byte[]> highScaleFallbackSourceEntries = null;
            ...
            if (hasVisualScaleSpec)
            {
                ...
                {
                    cleanReferenceFdtPath = visualSourceFdtPath;
            ...
            int allocationFailures = 0;
            int fallbackAllocations = 0;
            int reclaimedAllocations = 0;
            int directSourceAllocations = 0;
            for (int codepointIndex = 0; codepointIndex < requiredCodepoints.Length; codepointIndex++)
            {
                uint codepoint = requiredCodepoints[codepointIndex];
                if (!IsHangulCodepoint(codepoint))
                ...
                {
                    if (!useVisualSource ||
                        string.Equals(visualSourceFdtPath, baseSourceFdtPath, StringComparison.OrdinalIgnoreCase) ||
                        !baseSourceEntries.TryGetValue(utf8Value, out sourceEntryBytes))
                    {
                        missingSources++;
                        continue;
                    }
                ...
                bool directSourceAllocation = false;
                ...
                if (!applyVisualScale &&
                    TryUseCleanEmptyLobbySourceCell(
                        activeGlyphRepair,
                        globalArchive,
                        sourceTexturePath,
                        sourceEntry,
                        lobbyTextureCandidates,
                        targetTextures,
                        activeTexturePatches,
                        patchClearWidth,
                        patchClearHeight,
                        out allocation))
                {
                    if (LobbyAllocationFits(activeGlyphRepair, allocation, patchClearWidth, patchClearHeight))
                    {
                        hasAllocation = true;
                        addTexturePatch = true;
                        directSourceAllocation = true;
                    }
                }
                else if (hasVisualScaleSpec && visualScaleSpec.UsePlacementCells)
                {
                    ...
                    placementCell.ClearWidth = placementEntry.Width;
                    placementCell.ClearHeight = placementEntry.Height;
                    ...
                    activeLobbyHangulAllocationCache.TryGet(allocationKey, out allocation))
                {
                    hasAllocation = LobbyTextureCandidateContains(lobbyTextureCandidates, allocation.TexturePath) &&
                        LobbyAllocationFits(activeGlyphRepair, allocation, patchClearWidth, patchClearHeight);
                }
                ...
                if (!hasAllocation)
                {
                    ...
                    if (!TryAllocateLobbyHangulGlyphCell(
                        ...
                        out allocatedCell))
                    {
                        byte[] fallbackEntryBytes;
                        LobbyHangulGlyphAllocation fallbackAllocation;
                        if (IsHighScaleLobbyFont(normalizedPath) &&
                            activeLobbyHangulAllocationCache != null &&
                            TryGetHighScaleLobbyFallbackEntry(
                                payloadsByPath,
                                mpdStream,
                                utf8Value,
                                ref highScaleFallbackSourceLoaded,
                                ref highScaleFallbackSourceEntries,
                                out fallbackEntryBytes) &&
                            activeLobbyHangulAllocationCache.TryGetBySourceCodepoint(
                                HighScaleLobbyFallbackSourceFontPath,
                                codepoint,
                                out fallbackAllocation) &&
                            LobbyTextureCandidateContains(lobbyTextureCandidates, fallbackAllocation.TexturePath) &&
                            LobbyAllocationFits(activeGlyphRepair, fallbackAllocation, sourceEntry.Width, sourceEntry.Height))
                        {
                            sourceEntryBytes = fallbackEntryBytes;
                            sourceEntry = ReadFdtGlyphEntry(sourceEntryBytes, 0);
                            replacementWidth = sourceEntry.Width;
                            replacementHeight = sourceEntry.Height;
                            replacementOffsetX = sourceEntry.OffsetX;
                            replacementOffsetY = sourceEntry.OffsetY;
                            allocation = fallbackAllocation;
                            hasAllocation = true;
                            addTexturePatch = false;
                            fallbackAllocations++;
                        }
                        else
                        {
            ...
            if (changed > 0)
            {
                if (transactionalFontPatch && allocationFailures > 0)
                {
                    AddLimitedWarning(
                        "Skipped partial high-scale lobby Hangul glyphs for " +
                        normalizedPath +
                        ": " +
                        allocationFailures.ToString() +
                        " allocation failures");
                    return 0;
                }

                if (transactionalFontPatch)
                {
                    glyphRepair.CopyFrom(activeGlyphRepair);
                    if (lobbyHangulAllocationCache != null && activeLobbyHangulAllocationCache != null)
                    {
                        lobbyHangulAllocationCache.CopyFrom(activeLobbyHangulAllocationCache);
                    }

                    MergeTexturePatches(texturePatches, activeTexturePatches);
                }
            ...
            {
                AddLimitedWarning("Lobby high-scale fallback glyph allocations for " + normalizedPath + ": " + fallbackAllocations.ToString());
            }

            if (reclaimedAllocations > 0)
            {
                AddLimitedWarning("Lobby clean-page CJK reclaim glyph allocations for " + normalizedPath + ": " + reclaimedAllocations.ToString());
            }

            if (directSourceAllocations > 0)
            {
                AddLimitedWarning("Lobby clean-empty source-cell glyph allocations for " + normalizedPath + ": " + directSourceAllocations.ToString());
            }

            return changed;
        }

        private int ApplyHighScaleLobbyCleanAsciiGlyphs(
            string path,
            ref byte[] targetFdt,
            FontGlyphRepairContext glyphRepair,
            SqPackArchive globalArchive,
            Dictionary<string, List<FontTexturePatch>> texturePatches)
        {
            ...
            string[] sourceFdtPaths = GetHighScaleLobbyCleanAsciiSourceFdtPaths(normalizedPath);
            if (sourceFdtPaths.Length == 0)
            {
                return 0;
            }

            int glyphEnd = checked(glyphStart + checked((int)glyphCount) * FdtGlyphEntrySize);
            uint[] requiredAsciiCodepoints = CreateHighScaleLobbyCleanAsciiCodepoints();
            if (requiredAsciiCodepoints.Length == 0)
            {
                return 0;
            }

            List<byte[]> targetEntries = new List<byte[]>(checked((int)glyphCount + requiredAsciiCodepoints.Length));
            Dictionary<uint, int> targetEntryIndexes = new Dictionary<uint, int>();
            for (int glyphIndex = 0; glyphIndex < glyphCount; glyphIndex++)
            {
                byte[] entry = new byte[FdtGlyphEntrySize];
            ...
            Dictionary<string, Dictionary<uint, byte[]>> sourceEntriesByPath =
                new Dictionary<string, Dictionary<uint, byte[]>>(StringComparer.OrdinalIgnoreCase);
            Dictionary<string, byte[]> sourceTextures = new Dictionary<string, byte[]>(StringComparer.OrdinalIgnoreCase);
            string[] textureCandidates = GetLobbyHangulTextureCandidates(normalizedPath);
            int changed = 0;
            int missingSources = 0;
            int textureFailures = 0;
            int allocationFailures = 0;
            for (int codepointIndex = 0; codepointIndex < requiredAsciiCodepoints.Length; codepointIndex++)
            {
                uint codepoint = requiredAsciiCodepoints[codepointIndex];
                uint utf8Value = PackFdtUtf8Value(codepoint);
                if (targetEntryIndexes.ContainsKey(utf8Value))
                {
                    continue;
                ...
                byte[] sourceEntryBytes;
                if (!TryGetHighScaleLobbyCleanAsciiSourceEntry(
                        globalArchive,
                        sourceEntriesByPath,
                        sourceFdtPaths,
                        utf8Value,
                        out sourceFdtPath,
                        out sourceEntryBytes))
                {
                    missingSources++;
                    continue;
                }
                ...
                byte[] sourceTexture;
                if (sourceTexturePath == null ||
                    !TryReadCachedRawTexture(globalArchive, sourceTextures, sourceTexturePath, out sourceTexture))
                {
                    textureFailures++;
                    continue;
                }
                ...
                try
                {
                    sourceRegion = ExtractFontTextureAlphaRegion(sourceTexture, sourceEntry, CleanAsciiTexturePadding);
                }
                catch (ArgumentOutOfRangeException)
                {
                    textureFailures++;
                ...
                AllocatedFontGlyphCell allocatedCell;
                string allocatedTexturePath;
                if (!TryAllocateFromCandidateTextures(
                        glyphRepair,
                        textureCandidates,
                        sourceRegion.Width,
                        sourceRegion.Height,
                ...
                byte[] replacementEntry = new byte[FdtGlyphEntrySize];
                Buffer.BlockCopy(sourceEntryBytes, 0, replacementEntry, 0, FdtGlyphEntrySize);
                Endian.WriteUInt32LE(replacementEntry, 0, utf8Value);
                ushort shiftJis;
                if (TryEncodeShiftJisValue(codepoint, out shiftJis))
                {
                    Endian.WriteUInt16LE(replacementEntry, 4, shiftJis);
                }
            ...
            if (missingSources > 0)
            {
                AddLimitedWarning("High-scale lobby clean ASCII source glyphs missing for " + normalizedPath + ": " + missingSources.ToString());
            }

            if (textureFailures > 0)
            {
                AddLimitedWarning("High-scale lobby clean ASCII texture failures for " + normalizedPath + ": " + textureFailures.ToString());
            }

            if (allocationFailures > 0)
            {
                AddLimitedWarning("High-scale lobby clean ASCII atlas allocation failures for " + normalizedPath + ": " + allocationFailures.ToString());
            }
        ...
        private static void AddAsciiPhraseCodepoints(HashSet<uint> codepoints, string[] phrases)
        {
            ...
            for (int phraseIndex = 0; phraseIndex < phrases.Length; phraseIndex++)
            {
                ...
                for (int i = 0; i < phrase.Length; i++)
                {
                    ...
                    if (codepoint > 0x20u && codepoint <= (uint)CleanAsciiLast)
                    {
        ...
        private static string[] GetHighScaleLobbyCleanAsciiSourceFdtPaths(string targetPath)
        {
            string normalized = NormalizeGamePath(targetPath);
            List<string> paths = new List<string>();
            paths.Add(normalized);
            string fallback = ResolveDerived4kLobbySourceFdtPath(normalized);
            if (!string.IsNullOrEmpty(fallback) &&
                !string.Equals(fallback, normalized, StringComparison.OrdinalIgnoreCase))
            {
                paths.Add(fallback);
            }
        ...
        private static bool TryGetHighScaleLobbyCleanAsciiSourceEntry(
            SqPackArchive globalArchive,
            Dictionary<string, Dictionary<uint, byte[]>> sourceEntriesByPath,
            string[] sourceFdtPaths,
            uint utf8Value,
            out string sourceFdtPath,
            out byte[] sourceEntryBytes)
        {
            sourceFdtPath = null;
            sourceEntryBytes = null;
            for (int sourceIndex = 0; sourceIndex < sourceFdtPaths.Length; sourceIndex++)
            {
                string candidate = NormalizeGamePath(sourceFdtPaths[sourceIndex]);
                Dictionary<uint, byte[]> sourceEntries;
                if (!sourceEntriesByPath.TryGetValue(candidate, out sourceEntries))
                {
                    byte[] sourceFdt = TryReadArchiveStandardFile(globalArchive, candidate);
                    sourceEntries = sourceFdt == null
                        ? new Dictionary<uint, byte[]>()
                        : ReadGlyphEntriesByUtf8Value(sourceFdt);
                    sourceEntriesByPath.Add(candidate, sourceEntries);
                }

                if (sourceEntries.TryGetValue(utf8Value, out sourceEntryBytes))
                {
                    sourceFdtPath = candidate;
                    return true;
                }
            }
        ...
        private static int ApplyHighScaleLobbyCleanAsciiKerning(string path, ref byte[] targetFdt)
        {
            if (!IsHighScaleLobbyFont(path) || targetFdt == null)
            {
                return 0;
            }

            const int numericBoundaryAdjustment = 8;
            ...
            return UpsertKerningEntries(ref targetFdt, result, numericBoundaryAdjustment);
        }

        private static bool TryGetHighScaleLobbyFallbackEntry(
            Dictionary<string, FontPayload> payloadsByPath,
            FileStream mpdStream,
            uint utf8Value,
            ref bool fallbackSourceLoaded,
            ref Dictionary<uint, byte[]> fallbackSourceEntries,
            out byte[] entryBytes)
        {
            entryBytes = null;
            if (!fallbackSourceLoaded)
            {
                fallbackSourceLoaded = true;
                byte[] fallbackFdt = TryLoadTtmpStandardPayload(
                    payloadsByPath,
                    mpdStream,
                    HighScaleLobbyFallbackSourceFontPath);
                fallbackSourceEntries = fallbackFdt == null
                    ? new Dictionary<uint, byte[]>()
                    : ReadGlyphEntriesByUtf8Value(fallbackFdt);
            }

            return fallbackSourceEntries != null &&
                   fallbackSourceEntries.TryGetValue(utf8Value, out entryBytes);
        }
        ...
        private static string[] GetLobbyHangulTextureCandidates(string fdtPath)
        {
        ...
        private static bool TryUseCleanEmptyLobbySourceCell(
            ...
            string[] lobbyTextureCandidates,
            Dictionary<string, byte[]> cleanTextureCache,
            Dictionary<string, List<FontTexturePatch>> pendingTexturePatches,
            int clearWidth,
            int clearHeight,
            out LobbyHangulGlyphAllocation allocation)
        {
            allocation = new LobbyHangulGlyphAllocation();
            string normalizedTexturePath = NormalizeGamePath(sourceTexturePath);
            if (globalArchive == null ||
                !IsLobbyFontTexturePath(normalizedTexturePath) ||
                IsRuntimeUnsafeLobbyTexturePath(normalizedTexturePath) ||
                !LobbyTextureCandidateContains(lobbyTextureCandidates, normalizedTexturePath) ||
                cleanTextureCache == null)
            {
                return false;
            }

            byte[] cleanTexture;
            if (!TryReadCachedRawTexture(globalArchive, cleanTextureCache, normalizedTexturePath, out cleanTexture))
            {
                return false;
            }

            FontTexture texture = ReadFontTexture(cleanTexture);
            int targetX = sourceEntry.X;
            int targetY = sourceEntry.Y;
            int targetChannel = sourceEntry.ImageIndex % 4;
            int width = Math.Max(clearWidth, sourceEntry.Width);
            int height = Math.Max(clearHeight, sourceEntry.Height);
            if (targetX < 0 ||
                targetY < 0 ||
                targetX + width > texture.Width ||
                targetY + height >= texture.Height)
            {
                return false;
            }
            ...
            for (int y = 0; y < height; y++)
            {
                ...
                {
                    if (GetFontTextureChannel(texture, targetX + x, targetY + y, targetChannel) != 0)
                    {
                        return false;
                    }
                }
            ...
            FontTexturePatch probe = new FontTexturePatch();
            probe.TargetX = targetX;
            probe.TargetY = targetY;
            probe.TargetChannel = targetChannel;
            probe.ClearWidth = width;
            probe.ClearHeight = height;
            probe.SourceWidth = width;
            probe.SourceHeight = height;
            List<FontTexturePatch> existingPatches;
            if (pendingTexturePatches != null &&
                pendingTexturePatches.TryGetValue(normalizedTexturePath, out existingPatches))
            {
                for (int i = 0; i < existingPatches.Count; i++)
                {
                    if (OverlapsTexturePatch(probe, existingPatches[i]))
                    {
                        return false;
                    }
                }
            ...
            AllocatedFontGlyphCell cell = new AllocatedFontGlyphCell();
            cell.ImageIndex = sourceEntry.ImageIndex;
            cell.X = targetX;
            cell.Y = targetY;
            cell.Channel = targetChannel;
        ...
        {
            if (!EnableLobbyHangulAllocatedGlyphs)
            {
                return false;
            }
            ...
            out AllocatedFontGlyphCell allocatedCell)
        {
            ...
            string[] preferredCandidates = textureCandidates == null || textureCandidates.Length == 0
                ? GetPreferredLobbyTextureCandidates(normalizedFdtPath)
                : textureCandidates;
            return TryAllocateFromCandidateTextures(
                glyphRepair,
                preferredCandidates,
        ...
        private static string[] GetCleanLobbyTextureCandidates(SqPackArchive globalArchive, string fdtPath)
        {
            string normalizedFdtPath = NormalizeGamePath(fdtPath);
            if (globalArchive == null ||
                normalizedFdtPath.IndexOf("_lobby.fdt", StringComparison.OrdinalIgnoreCase) < 0)
            {
                return GetPreferredLobbyTextureCandidates(normalizedFdtPath);
            }

            byte[] cleanFdt = TryReadArchiveStandardFile(globalArchive, normalizedFdtPath);
            if (cleanFdt == null)
            {
                return GetPreferredLobbyTextureCandidates(normalizedFdtPath);
            }
            ...
            HashSet<string> cleanPages = new HashSet<string>(StringComparer.OrdinalIgnoreCase);
            for (int glyphIndex = 0; glyphIndex < glyphCount; glyphIndex++)
            {
                FdtGlyphEntry glyph = ReadFdtGlyphEntry(cleanFdt, glyphStart + glyphIndex * FdtGlyphEntrySize);
                if (glyph.Width == 0 || glyph.Height == 0)
                {
                    continue;
                }

                string texturePath = ResolveFontTexturePath(normalizedFdtPath, glyph.ImageIndex);
                if (IsLobbyFontTexturePath(texturePath))
                {
                    cleanPages.Add(NormalizeGamePath(texturePath));
                }
            }
            ...
            string[] preferred = GetPreferredLobbyTextureCandidates(normalizedFdtPath);
            List<string> filtered = new List<string>();
            for (int i = 0; i < preferred.Length; i++)
            {
                string candidate = NormalizeGamePath(preferred[i]);
                if (cleanPages.Contains(candidate))
                {
        ...
        private static bool ShouldRepairCleanAsciiFont(string path)
        {
            ...
            return IsCleanDamageNumberFontPath(normalized) ||
                   ...
                   ShouldRepairCleanFullAsciiTrumpGothicFont(normalized);
        }
        ...
        private static bool ShouldRepairCleanFullAsciiTrumpGothicFont(string path)
        {
            string normalized = NormalizeGamePath(path);
            return string.Equals(normalized, "common/font/TrumpGothic_34.fdt", StringComparison.OrdinalIgnoreCase) ||
        ...
        private static void AddTexturePatch(Dictionary<string, List<FontTexturePatch>> patchesByPath, string texturePath, FontTexturePatch patch)
        {
            ...
            if (!patchesByPath.TryGetValue(normalizedPath, out patches))
            {
                patches = new List<FontTexturePatch>();
                patchesByPath.Add(normalizedPath, patches);
            }

            patches.Add(patch);
        }

        private static void MergeTexturePatches(
            Dictionary<string, List<FontTexturePatch>> target,
            Dictionary<string, List<FontTexturePatch>> source)
        {
            ...
            foreach (KeyValuePair<string, List<FontTexturePatch>> pair in source)
            {
                List<FontTexturePatch> patches = pair.Value;
                if (patches == null || patches.Count == 0)
                {
                    continue;
                }

                List<FontTexturePatch> targetPatches;
                string normalizedPath = NormalizeGamePath(pair.Key);
                if (!target.TryGetValue(normalizedPath, out targetPatches))
                {
                    targetPatches = new List<FontTexturePatch>();
                    target.Add(normalizedPath, targetPatches);
                }

                targetPatches.AddRange(patches);
            }
        }
        ...
        private LobbyHangulCodepointSets CreateLobbyHangulCodepointSets(
            string patchedTextSqpack,
            string fallbackTextSqpack)
        {
            ...
            uint[] systemSettingsRaw = CreateLobbyHangulCodepoints(
                patchedTextSqpack,
                fallbackTextSqpack,
        ...
        private static bool IsSystemSettingsLobbyAxisFont(string path)
        {
            string normalized = NormalizeGamePath(path);
            return string.Equals(normalized, "common/font/AXIS_12_lobby.fdt", StringComparison.OrdinalIgnoreCase) ||
                   string.Equals(normalized, "common/font/AXIS_14_lobby.fdt", StringComparison.OrdinalIgnoreCase) ||
        ...
        {
            if (codepoints == null ||
                string.IsNullOrEmpty(textSqpack) ||
                string.IsNullOrEmpty(fallbackTextSqpack) ||
                ranges == null ||
                ranges.Length == 0)
            {
                return 0;
            ...
            try
            {
                ...
                using (SqPackArchive fallbackArchive = new SqPackArchive(fallbackTextIndexPath, fallbackTextSqpack, TextDatPrefix))
                {
                    ...
                    {
                        AddLobbySheetHangulCodepointsFromSheet(textArchive, fallbackArchive, codepoints, language, pair.Key, pair.Value, label);
                    }
                }
            }
            catch (Exception ex)
        ...
        private static bool TryDecodeFdtGlyphCodepoint(byte[] entry, int offset, out uint codepoint)
        {
            codepoint = 0;
            if (entry == null || offset < 0 || offset > entry.Length - FdtGlyphEntrySize)
            {
                return false;
            }

            uint utf8Value = Endian.ReadUInt32LE(entry, offset);
            if (TryDecodeFdtUtf8Value(utf8Value, out codepoint))
            {
                return true;
            }

            ushort shiftJis = Endian.ReadUInt16LE(entry, offset + 4);
            return TryDecodeShiftJisValue(shiftJis, out codepoint);
        }

        private static bool TryDecodeShiftJisValue(ushort shiftJis, out uint codepoint)
        {
            codepoint = 0;
            if (shiftJis == 0)
            {
            ...
            byte[] bytes = shiftJis <= 0xFF
                ? new byte[] { (byte)shiftJis }
                : new byte[] { (byte)(shiftJis >> 8), (byte)(shiftJis & 0xFF) };
            try
            {
                ...
                codepoint = (uint)char.ConvertToUtf32(text, 0);
                return true;
            }
        ...
        private static FontGlyphRepairContext TryCreateFontGlyphRepairContext(FontPatchPackage fontPackage, FileStream mpdStream, SqPackArchive globalArchive, bool fontOnly)
        {
            ...
            AddLobbyCleanCellReclaimPools(context, globalArchive);
        ...
        private static void AddLobbyCleanCellReclaimPools(FontGlyphRepairContext context, SqPackArchive globalArchive)
        {
            if (context == null || globalArchive == null)
            {
                return;
            ...
            {
                string fontPath = NormalizeGamePath(LobbyHangulCoverage.TargetFontPaths[i]);
            ...
            SqPackArchive globalArchive)
        {
            ...
            for (int i = 0; i < FontPaths.Length; i++)
            {
                string fontPath = NormalizeGamePath(FontPaths[i]);
                if (!IsLobbyFontPath(fontPath))
                {
                    continue;
                }
                ...
                for (int glyphIndex = 0; glyphIndex < glyphCount; glyphIndex++)
                {
                    ...
                    uint codepoint;
                    if (!TryDecodeFdtUtf8Value(Endian.ReadUInt32LE(fdt, offset), out codepoint) ||
                        !IsProtectedCleanLobbyGlyphCodepoint(codepoint))
                    {
                        continue;
                    }
                    ...
                    FdtGlyphEntry glyph = ReadFdtGlyphEntry(fdt, offset);
                    ReclaimableFontGlyphCell cell;
                    if (!TryCreateCleanLobbyCell(context, fontPath, glyph, out cell))
                    {
                        continue;
                    }

                    cell = InflateCleanLobbyProtectedCell(context, cell, codepoint);
                    List<ReclaimableFontGlyphCell> cells;
                    if (!protectedCells.TryGetValue(cell.TexturePath, out cells))
                    {
                        cells = new List<ReclaimableFontGlyphCell>();
                        protectedCells.Add(cell.TexturePath, cells);
                    }
        ...
        private static ReclaimableFontGlyphCell InflateCleanLobbyProtectedCell(
            FontGlyphRepairContext context,
            ReclaimableFontGlyphCell cell,
            uint codepoint)
        {
            int padding = IsCleanAsciiCodepoint(codepoint)
                ? CleanAsciiTexturePadding
                : 0;
            if (padding <= 0)
            {
            ...
            FontAtlasAllocator allocator;
            if (context == null || !context.TryGetAllocator(cell.TexturePath, out allocator))
            {
                return cell;
            }

            int left = Math.Max(0, cell.X - padding);
            int top = Math.Max(0, cell.Y - padding);
            int right = Math.Min(allocator.Width, cell.X + cell.Width + padding);
            int bottom = Math.Min(allocator.Height, cell.Y + cell.Height + padding);
            cell.X = left;
            cell.Y = top;
            cell.Width = Math.Max(0, right - left);
            cell.Height = Math.Max(0, bottom - top);
            return cell;
        }
            ...
            out ReclaimableFontGlyphCell cell)
        {
            ...
            if (!TryCreateCleanLobbyCell(context, fontPath, glyph, out cell) ||
                !LobbyTextureCandidateContains(cleanPages, cell.TexturePath) ||
                CleanLobbyCellTouchesBottomEdge(context, cell) ||
                CleanLobbyCellOverlapsProtectedCell(cell, protectedCells))
            {
                return false;
            }
        ...
        private static bool TryCreateCleanLobbyCell(
            FontGlyphRepairContext context,
            string fontPath,
            FdtGlyphEntry glyph,
            out ReclaimableFontGlyphCell cell)
        ...
        private static bool CleanLobbyCellOverlapsProtectedCell(
            ReclaimableFontGlyphCell cell,
            Dictionary<string, List<ReclaimableFontGlyphCell>> protectedCells)
        {
            ...
            List<ReclaimableFontGlyphCell> cells;
            if (!protectedCells.TryGetValue(NormalizeGamePath(cell.TexturePath), out cells))
            {
                return false;
            }
            ...
            {
                ReclaimableFontGlyphCell protectedCell = cells[i];
                if (cell.Channel == protectedCell.Channel &&
                    RectanglesOverlap(
                        cell.X,
                        cell.Y,
                        cell.Width,
        ...
        private static bool CleanLobbyCellTouchesBottomEdge(FontGlyphRepairContext context, ReclaimableFontGlyphCell cell)
        {
            FontAtlasAllocator allocator;
            return context != null &&
                   context.TryGetAllocator(cell.TexturePath, out allocator) &&
                   cell.Y + cell.Height >= allocator.Height;
        }

        private static bool IsProtectedCleanLobbyGlyphCodepoint(uint codepoint)
        {
            return !IsReclaimableCleanLobbyGlyphCodepoint(codepoint);
        }

        private static bool IsReclaimableCleanLobbyGlyphCodepoint(uint codepoint)
        {
            return (codepoint >= 0x3041u && codepoint <= 0x3096u) ||
                   (codepoint >= 0x30A1u && codepoint <= 0x30FAu) ||
                   (codepoint >= 0x3400u && codepoint <= 0x4DBFu) ||
                   (codepoint >= 0x4E00u && codepoint <= 0x9FFFu);
        }

        private static bool IsCleanAsciiCodepoint(uint codepoint)
        {
            return codepoint >= CleanAsciiFirst && codepoint <= CleanAsciiLast;
        }

        private static bool IsRuntimeUnsafeLobbyTexturePath(string texturePath)
        {
            return string.Equals(
                NormalizeGamePath(texturePath),
        ...
        private static int GetReservedFontGlyphTextureNeighborhoodPadding(string fdtPath, string texturePath, uint codepoint)
        {
            ...
            {
                return IsCleanAsciiCodepoint(codepoint)
                    ? CleanAsciiTexturePadding
                    : LobbyGlyphTextureNeighborhoodPadding;
            }
                ...
                for (int y = sourceTop; y < sourceBottom; y++)
                {
                    ...
                    for (int x = sourceLeft; x < sourceRight; x++)
                    {
                        ...
                        for (int sourceY = baseY0; sourceY < baseY1; sourceY++)
                        {
                            ...
                            {
                                byte alpha = patch.SourceAlpha[row + sourceX];
                                if (alpha > maxAlpha)
                                {
                                    maxAlpha = alpha;
                                }
                        ...
                        SetFontTextureChannel(texture, x, y, patch.TargetChannel, (byte)(maxAlpha / 17), level);
                    }
                }
            }
        }
        ...
        private sealed class FontGlyphRepairContext
        {
            ...
            public bool IsFree(string texturePath, int x, int y, int width, int height, int channel)
            {
                FontAtlasAllocator allocator;
                if (!_allocators.TryGetValue(NormalizeGamePath(texturePath), out allocator))
                {
                ...
                return allocator.IsFree(x, y, width, height, channel);
            }
                ...
                out AllocatedFontGlyphCell allocatedCell)
            {
                ...
                ReclaimableFontGlyphCell selected = cells[bestIndex];
                selected.Used = true;
                cells[bestIndex] = selected;
                MarkReclaimedLobbyCellUsed(selected);
                for (int i = 0; i < cells.Count; i++)
                {
                    if (i == bestIndex)
                    {
                ...
                allocatedCell.ClearWidth = selected.Width;
                allocatedCell.ClearHeight = selected.Height;
            ...
            private bool ReclaimableCellOverlapsUsedCell(ReclaimableFontGlyphCell candidate)
            {
                ...
                if (!_usedReclaimedLobbyCellsByTexture.TryGetValue(NormalizeGamePath(candidate.TexturePath), out usedCells))
                {
                    return false;
                }
                ...
                for (int i = 0; i < usedCells.Count; i++)
                {
                    ...
                    if (candidate.Channel == used.Channel &&
                        RectanglesOverlap(
                            candidate.X,
                            candidate.Y,
                            candidate.Width,
            ...
            public FontGlyphRepairContext Clone()
            {
                FontGlyphRepairContext clone = new FontGlyphRepairContext();
                foreach (KeyValuePair<string, FontAtlasAllocator> pair in _allocators)
                {
                    clone._allocators.Add(pair.Key, pair.Value.Clone());
                }

                foreach (KeyValuePair<string, List<ReclaimableFontGlyphCell>> pair in _reclaimableLobbyCellsByFont)
                {
                    clone._reclaimableLobbyCellsByFont.Add(pair.Key, CloneReclaimableCells(pair.Value));
                }

                foreach (KeyValuePair<string, List<ReclaimableFontGlyphCell>> pair in _usedReclaimedLobbyCellsByTexture)
                {
                    clone._usedReclaimedLobbyCellsByTexture.Add(pair.Key, CloneReclaimableCells(pair.Value));
                }

                return clone;
            }

            public void CopyFrom(FontGlyphRepairContext source)
            {
                if (source == null)
                {
                    return;
                ...
                _allocators.Clear();
                foreach (KeyValuePair<string, FontAtlasAllocator> pair in source._allocators)
                {
                    _allocators.Add(pair.Key, pair.Value.Clone());
                }

                _reclaimableLobbyCellsByFont.Clear();
                foreach (KeyValuePair<string, List<ReclaimableFontGlyphCell>> pair in source._reclaimableLobbyCellsByFont)
                {
                    _reclaimableLobbyCellsByFont.Add(pair.Key, CloneReclaimableCells(pair.Value));
                }

                _usedReclaimedLobbyCellsByTexture.Clear();
                foreach (KeyValuePair<string, List<ReclaimableFontGlyphCell>> pair in source._usedReclaimedLobbyCellsByTexture)
                {
                    _usedReclaimedLobbyCellsByTexture.Add(pair.Key, CloneReclaimableCells(pair.Value));
                }
            }

            private static List<ReclaimableFontGlyphCell> CloneReclaimableCells(List<ReclaimableFontGlyphCell> cells)
            {
                List<ReclaimableFontGlyphCell> clone = new List<ReclaimableFontGlyphCell>();
                if (cells == null)
                {
                    return clone;
                }
        ...
        private sealed class LobbyHangulGlyphAllocationCache
        {
            ...
            private readonly Dictionary<string, LobbyHangulGlyphAllocation> _allocationsBySourceCodepoint =
                new Dictionary<string, LobbyHangulGlyphAllocation>(StringComparer.OrdinalIgnoreCase);
            ...
            public bool TryGetBySourceCodepoint(string sourceFdtPath, uint codepoint, out LobbyHangulGlyphAllocation allocation)
            {
                return _allocationsBySourceCodepoint.TryGetValue(CreateSourceCodepointKey(sourceFdtPath, codepoint), out allocation);
            }

            public void Add(string key, string sourceFdtPath, uint codepoint, LobbyHangulGlyphAllocation allocation)
            {
                ...
                string sourceKey = CreateSourceCodepointKey(sourceFdtPath, codepoint);
                if (!_allocationsBySourceCodepoint.ContainsKey(sourceKey))
                {
                    _allocationsBySourceCodepoint.Add(sourceKey, allocation);
                }
            }

            public LobbyHangulGlyphAllocationCache Clone()
            {
                LobbyHangulGlyphAllocationCache clone = new LobbyHangulGlyphAllocationCache();
                foreach (KeyValuePair<string, LobbyHangulGlyphAllocation> pair in _allocations)
                {
                    clone._allocations.Add(pair.Key, pair.Value);
                }

                foreach (KeyValuePair<string, LobbyHangulGlyphAllocation> pair in _allocationsBySourceCodepoint)
                {
                    clone._allocationsBySourceCodepoint.Add(pair.Key, pair.Value);
                }

                return clone;
            }

            public void CopyFrom(LobbyHangulGlyphAllocationCache source)
            {
                if (source == null)
                {
                    return;
                ...
                _allocations.Clear();
                foreach (KeyValuePair<string, LobbyHangulGlyphAllocation> pair in source._allocations)
                {
                    _allocations.Add(pair.Key, pair.Value);
                }

                _allocationsBySourceCodepoint.Clear();
                foreach (KeyValuePair<string, LobbyHangulGlyphAllocation> pair in source._allocationsBySourceCodepoint)
                {
                    _allocationsBySourceCodepoint.Add(pair.Key, pair.Value);
                }
            }

            private static string CreateSourceCodepointKey(string sourceFdtPath, uint codepoint)
            {
                return NormalizeGamePath(sourceFdtPath) + "|" + codepoint.ToString("X8");
            }
        }
        ...
        private sealed class FontAtlasAllocator
        {
            ...
            private FontAtlasAllocator(
                FontTexture texture,
                ulong[][][] occupiedBits,
                Dictionary<ulong, int> denseScanCursors,
                int wordsPerRow)
            {
                _texture = texture;
                _occupiedBits = occupiedBits;
                _denseScanCursors = denseScanCursors;
                _wordsPerRow = wordsPerRow;
            }

            public FontAtlasAllocator Clone()
            {
                ulong[][][] occupiedBits = new ulong[_occupiedBits.Length][][];
                for (int channel = 0; channel < _occupiedBits.Length; channel++)
                {
                    occupiedBits[channel] = new ulong[_occupiedBits[channel].Length][];
                    for (int y = 0; y < _occupiedBits[channel].Length; y++)
                    {
                        occupiedBits[channel][y] = new ulong[_occupiedBits[channel][y].Length];
                        Array.Copy(
                            _occupiedBits[channel][y],
                            occupiedBits[channel][y],
                            _occupiedBits[channel][y].Length);
                    }
                }

                return new FontAtlasAllocator(
                    _texture,
                    occupiedBits,
                    new Dictionary<ulong, int>(_denseScanCursors),
                    _wordsPerRow);
            }
            ...
            public bool IsFree(int x, int y, int width, int height, int channel)
            {
                if (channel < 0 || channel >= _occupiedBits.Length ||
                    x < 0 ||
                    y < 0 ||
                    width < 0 ||
                    height < 0 ||
                    x + width > _texture.Width ||
                    y + height > _texture.Height)
                {
                    return false;
                }
