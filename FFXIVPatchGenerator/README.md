...
기본값은 `full`입니다. FDT 파일은 글로벌 원본 glyph를 기준으로 두고 한국어 Hangul glyph만 overlay에서 병합하므로 숫자/영문/기호/UI glyph는 글로벌 원본을 유지합니다. 나머지 프로필은 파티 리스트 숫자처럼 특정 UI glyph가 깨질 때 원인이 되는 폰트군을 찾기 위한 진단용입니다. UI에서는 테스트 빌드에서만 선택할 수 있습니다.
