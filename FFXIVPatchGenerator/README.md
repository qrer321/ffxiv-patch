...
공용 비로비 UI에서 배율별로 함께 사용되는 `TrumpGothic_23/34/68.fdt`는 생성된 EXD 출력에 실제로 포함된 한글/자모를 동일한 시각 배율 규칙으로 보정합니다. 생성기는 EXD를 쓰는 기존 순회 중 글자 집합을 수집하고, 투명 여백을 제외한 큰 glyph부터 기존 atlas의 빈 영역에 배치합니다. 모든 대상 glyph를 안전하게 배치할 수 없으면 FDT/texture를 기록하기 전에 생성을 중단하며, texture 크기나 페이지 수를 임의로 늘리지 않습니다. 최종 EXD에 없는 임의 사용자 입력과 미래 문자열까지 11,172개 현대 한글 음절 전체를 선할당하지는 않습니다.
