...
기본값은 `full`입니다. 폰트 패치는 TTMP 패키지의 FDT와 texture를 한 세트로 그대로 사용하며, FDT 내부 glyph 좌표나 문자 코드를 글로벌 원본과 섞지 않습니다. FDT와 texture atlas가 서로 맞지 않으면 한글 폰트 전체가 깨질 수 있기 때문입니다. 나머지 프로필은 특정 UI glyph가 깨질 때 원인이 되는 폰트군을 찾기 위한 진단용입니다. UI에서는 테스트 빌드에서만 선택할 수 있습니다.
