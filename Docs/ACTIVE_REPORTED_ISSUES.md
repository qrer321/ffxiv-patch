...
- 2026-05-16: `applied-lobby-routes` compares generated output with `--applied-game` at ULD route and FDT/TEX payload level. D global folder currently has matching ULD routes but mismatching lobby/font payloads, so live-client failures must distinguish route/code bugs from stale or different installed payloads.
