...
- 2026-05-31 applied full visual gate evidence: the installed game folder also passed `verify-hd-crash-release.ps1 -SkipGenerate -AppliedGame ... -RunFullFontObjective -DumpVisualSnapshots -FailOnAnyNewCrash`; that full-objective run remains represented by the accepted objective logs below. The latest applied snapshot-only recheck is `.tmp\hd-crash-release-gate-logs\20260531-183600-visual-snapshots`; use it as the current PNG evidence set for lobby settings 150/200/300%, character-select large labels, ActionDetail `즉시 발동`/`초`, and PvP profile route checks.
...
- 2026-05-31 objective-log hygiene: the valid applied full-objective evidence set is `20260531-022850` through `20260531-024229`, all with applied `D:\...\game\sqpack\ffxiv` as patched sqpack and `RESULT: PASS`. Ignore `20260531-171804-objective-lobby-critical.log`; it came from an interrupted timeout run.
...
- 2026-05-31 18:22 recheck: the quick applied drift gate still passes against installed `D:\...\game`, and `Scripts\check-hd-crash-logs.ps1 -FailOnAnyNewCrash` finds no `AtkFontAnalyzerRenderer` appcrash newer than the `2026-05-31 01:08:12` release build.
...
- 2026-05-31 visual-report audit: `verify-hd-crash-release.ps1 -DumpVisualSnapshots` now runs `Scripts\check-visual-snapshot-reports.ps1` after PNG generation. The audit checks required lobby/UI/PvP report ids, verifies referenced PNG/PNG2 files exist, are non-empty, open as bitmaps, are at least the reported render size, and contain non-background pixels, then fails severe overlap/gap/fringe outliers. Applied run `20260531-183600-visual-snapshots` passed this audit.
- 2026-05-31 visual-audit self-test: `verify-hd-crash-release.ps1 -DumpVisualSnapshots` now runs `Scripts\test-visual-snapshot-report-audit.ps1` before checking the real visual reports. It proves valid reports pass and missing required ids, blank PNGs, severe overlap, and strict raw overlap fail; latest integrated applied run wrote `.tmp\visual-snapshot-audit-selftest\20260531-183934` and passed.
