...
- 2026-05-31 visual-report audit: `verify-hd-crash-release.ps1 -DumpVisualSnapshots` now runs `Scripts\check-visual-snapshot-reports.ps1` after PNG generation. The audit checks required lobby/UI/PvP report ids, verifies referenced PNG/PNG2 files exist, are non-empty, open as bitmaps, are at least the reported render size, and contain non-background pixels, then fails severe overlap/gap/fringe outliers. Applied run `20260531-181336-visual-snapshots` passed this audit.
- 2026-05-31 visual-audit self-test: `verify-hd-crash-release.ps1 -DumpVisualSnapshots` now runs `Scripts\test-visual-snapshot-report-audit.ps1` before checking the real visual reports. It proves valid reports pass and missing required ids, blank PNGs, and severe overlap fail; latest integrated applied run wrote `.tmp\visual-snapshot-audit-selftest\20260531-181713` and passed.
- 2026-05-31 raw visual overlap diagnostic: `Scripts\check-visual-snapshot-reports.ps1` prints bounded raw overlap/negative-gap rows and has `-FailOnAnyRawOverlap` for stricter investigation. The current applied snapshot set fails strict raw mode on bounded rows that are accepted by the C# verifier through clean/TTMP baseline or anti-alias tolerances, so do not change font metrics solely to make strict raw mode green; use it to localize any future live rejection.
