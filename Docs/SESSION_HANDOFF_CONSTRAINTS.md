...
- 2026-05-24 4K lobby coverage caveat: broad `4k-lobby-font-derivations` can still expose missing Hangul in high-scale lobby candidate fonts such as `Jupiter_46_lobby.fdt`. Do not conflate that visual/coverage failure with crash-class runtime bounds; keep it as a separate follow-up unless the live crash stack points at that lobby route.
...
- 2026-05-29 HD crash release gate: use `Scripts\verify-hd-crash-release.ps1` before reporting this issue. The short mode can reuse `.tmp\release-hd-crash-focus-ja` with `-SkipGenerate`; the full mode regenerates focused output and can add `-RunInGameCritical` for the long in-game regression sweep. The gate must pass focused HD crash checks, reject the known-bad `20260528-003930` output, and pass the post-release crash-log check.
...
- 2026-05-24 high-scale lobby ASCII rule: do not inject full ASCII into every high-scale lobby font. That can exhaust safe atlas space, especially `Meidinger_40_lobby.fdt`. Only add phrase-derived ASCII for the high-resolution UI/result-message routes, and add clean numeric-boundary kerning for digit-to-`%` and digit-to-`x` pairs when fallback ASCII glyphs are inserted.
...
- 2026-05-25 PvP 100% guard: do not add `common/font/Jupiter_16.fdt` back to `PvpProfileVisualScaleGlyphs` without a failing-then-passing verifier and live confirmation. Current rule treats `Jupiter_16` as the 100% source-preserved route and leaves `Jupiter_20` as the higher-scale visual route.
