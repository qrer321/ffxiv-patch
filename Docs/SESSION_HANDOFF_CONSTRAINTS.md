...
- 2026-05-30 source-preservation caveat: full `hangul-source-preservation` currently fails on the known `Jupiter_20.fdt` PvP visual-scale exception. Do not hide that failure, but do not use it as the blocker for start-screen PUA alias work until the PvP/Jupiter exception is modeled separately.
