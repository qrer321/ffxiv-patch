...
- 2026-05-30 source-preservation guard: full `hangul-source-preservation` must keep passing with the PvP visual-scale exception modeled as the route-scoped `PvpProfileVisualScaleGlyphs` codepoint set only. Do not replace it with a blanket `Jupiter_20.fdt` skip; run it with `pvp-profile-font-routes`, `third-party-game-font-safety`, and `ingame-ttmp-texture-neighborhoods` when touching this area.
