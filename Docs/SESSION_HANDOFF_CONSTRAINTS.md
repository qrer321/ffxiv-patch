...
- 2026-05-30 applied-state guard: `Scripts\verify-hd-crash-release.ps1` supports `-AppliedGame`. Use it when checking a release already applied to the game folder, so stale `.tmp` outputs are not compared against the installed client by mistake. The installed `D:\SquareEnix\FINAL FANTASY XIV - A Realm Reborn\game` currently matches `%LOCALAPPDATA%\FFXIVKoreanPatch\generated-release\ja\2026.05.01.0000.0000\20260530-002104`; that output passes current focused HD font checks, so the next crash fix needs either a new native stack or a verifier that fails this exact applied output before changing font placement.
