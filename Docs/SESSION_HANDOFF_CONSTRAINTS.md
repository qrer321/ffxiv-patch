...
- 2026-05-27 HD/non-4K startup crash guard: startup/runtime-limited small lobby fonts (`AXIS_12/14/18_lobby`, `MiedingerMid_12/14/18_lobby`) must not place Hangul on `font_lobby5/6` (`image_index >= 16`). The crash log points at `AtkFontAnalyzerRenderer` with the renderer buffer for texture slot 4 missing. Keep this as a narrow route guard; large-label and high-scale lobby fonts still use six clean lobby pages unless a verifier proves the route is unsafe.
