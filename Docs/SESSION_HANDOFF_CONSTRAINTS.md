...
- 2026-05-31 live-rejection correction: do not blame a clean installed game folder after the user has reverted a broken build. The broken generated output `20260531-214231` itself must fail. Current verifier fails it on patched Hangul routed through `font_lobby3.tex`, while allowing clean glyphs that already used that page. Fresh `.tmp\lobby-no-fontlobby3-ja-r1` passes `lobby-critical` and `ingame-critical`, but this is not live closure until user confirmation.
...
- 2026-05-31 applied full visual gate evidence: the installed game folder passed `verify-hd-crash-release.ps1 -SkipGenerate -AppliedGame ... -RunFullFontObjective -DumpVisualSnapshots -FailOnAnyNewCrash` on the latest run. Use `.tmp\hd-crash-release-gate-logs\20260531-184302-visual-snapshots` as the current PNG evidence set for lobby settings 150/200/300%, character-select large labels, ActionDetail `즉시 발동`/`초`, and PvP profile route checks.
- 2026-05-31 applied objective gate rule: `verify-hd-crash-release.ps1 -RunFullFontObjective -AppliedGame` must pass `--applied-game` and clean font/UI indexes into each captured objective chunk. The current hardened script does this and passed the applied re-run; do not treat older generated-output-only objective logs as equivalent applied-state evidence.
- 2026-05-31 weak applied-state check: `verify-hd-crash-release.ps1 -SkipGenerate -SkipFocusedChecks -AppliedGame ... -SkipKnownFailureCheck -SkipCrashLogCheck` is the quick post-apply drift check. It still runs `applied-output-files` and `applied-lobby-routes,lobby-runtime-font-safety,font-runtime-glyph-bounds`, but skips expensive focused/render checks. Use this for fast iteration only; final font claims still require `-RunFullFontObjective` and visual snapshots.
- 2026-05-31 objective-log hygiene: the valid applied full-objective evidence set is now `20260531-184641` through `20260531-185914`, all with applied `D:\...\game\sqpack\ffxiv` as patched sqpack and `RESULT: PASS`. Older accepted `20260531-022850` through `20260531-024229` logs are historical backup evidence. Ignore `20260531-171804-objective-lobby-critical.log`; it came from an interrupted timeout run.
- 2026-05-31 supersession note: older open/next-priority notes for lobby high-scale glyphs, ActionDetail large labels, combat flytext, and in-game blur/halo are retained as history, but the current code/applied-state evidence is the applied full visual gate plus the accepted objective logs above. Do not reopen those older notes as current failures unless a fresh client report or a new verifier failure contradicts the 2026-05-31 applied evidence. Still do not mark live-fixed until the user confirms the client result.
- 2026-05-31 19:00 recheck: the full applied gate still passes against installed `D:\...\game`, and `Scripts\check-hd-crash-logs.ps1 -FailOnAnyNewCrash` finds no `AtkFontAnalyzerRenderer` appcrash newer than the `2026-05-31 01:08:12` release build.
- 2026-05-31 completion-audit mapping: active objective evidence is now mapped in `Docs/ACTIVE_REPORTED_ISSUES.md` under "2026-05-31 completion-audit matrix". It ties HD/4K lobby, `-`/`=` fallback, runtime bounds, ActionDetail `즉시 발동`/`초`, PvP profile scale, UI-scale snapshots, installed drift, and crash logs to specific accepted logs/snapshots. Keep the goal open until live client confirmation, but use that matrix as the starting point for any future rejection.
- 2026-05-31 visual-report audit: `verify-hd-crash-release.ps1 -DumpVisualSnapshots` now runs `Scripts\check-visual-snapshot-reports.ps1` after PNG generation. The audit checks required lobby/UI/PvP report ids, verifies referenced PNG/PNG2 files exist, are non-empty, open as bitmaps, are at least the reported render size, and contain non-background pixels, then fails severe overlap/gap/fringe outliers. Applied run `20260531-184302-visual-snapshots` passed this audit.
- 2026-05-31 visual-audit self-test: `verify-hd-crash-release.ps1 -DumpVisualSnapshots` now runs `Scripts\test-visual-snapshot-report-audit.ps1` before checking the real visual reports. It proves valid reports pass and missing required ids, blank PNGs, severe overlap, and strict raw overlap fail; latest integrated applied run wrote `.tmp\visual-snapshot-audit-selftest\20260531-184640` and passed.
- 2026-05-31 raw visual overlap diagnostic: `Scripts\check-visual-snapshot-reports.ps1` prints bounded raw overlap/negative-gap rows and has `-FailOnAnyRawOverlap` for stricter investigation. The current applied snapshot set fails strict raw mode on bounded rows that are accepted by the C# verifier through clean/TTMP baseline or anti-alias tolerances, so do not change font metrics solely to make strict raw mode green; use it to localize any future live rejection. The latest integrated gate confirms these rows are visible in the normal PASS log.
