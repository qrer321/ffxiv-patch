...
- 2026-05-31 visual-report audit: `verify-hd-crash-release.ps1 -DumpVisualSnapshots` now runs `Scripts\check-visual-snapshot-reports.ps1` after PNG generation. The audit checks required lobby/UI/PvP report ids, verifies referenced PNG/PNG2 files exist, are non-empty, open as bitmaps, are at least the reported render size, and contain non-background pixels, then fails severe overlap/gap/fringe outliers. Applied run `20260531-174932-visual-snapshots` passed this audit.
- 2026-05-31 visual-audit self-test: run `Scripts\test-visual-snapshot-report-audit.ps1` when changing visual report checks. It proves valid reports pass and missing required ids, blank PNGs, and severe overlap fail; latest run wrote `.tmp\visual-snapshot-audit-selftest\20260531-180829` and passed.
