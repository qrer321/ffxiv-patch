...
- 2026-08-13 케프카 전투 시작 대사 재교정: U+E040/U+E041 문자를 직접 넣은 방식은 실제 상용구가 아니라 PUA 글리프였고, Completion `MacroCode.Fixed` 방식도 전투 대사 UI가 평가하지 않아 네이티브 상용구 표시를 재현하지 못했다. `InstanceContentTextData#45500`은 이제 대상 글로벌 언어의 원본 RSV 토큰을 행 전체로 보존한다. 이 한 행은 한국어 직역보다 클라이언트의 네이티브 상용구 해석과 표시를 우선하며, 나머지 케프카 RSV 대사는 계속 한국어로 치환한다. 회귀 검사는 생성 EXD가 `_rsv_45500_-1_0_0_0_S13095D61_E13095D61`(JA) 또는 대응 EN 토큰과 바이트 단위로 일치해야 통과하므로 literal Hangul, PUA, 색상 매크로, Fixed payload 재도입을 모두 차단한다.
