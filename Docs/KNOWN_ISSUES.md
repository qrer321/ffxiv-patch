...
- 2026-08-13 케프카 전투 시작 대사 재교정: 외부 `rsv.json`의 `InstanceContentTextData#45500` 한국어 값은 네이티브 RSV 처리 경로가 해석하는 자동번역 괄호 장식을 ASCII `7`/`8`로, SeString 줄바꿈 매크로를 `\n`으로 평탄화한다. 첫 교정은 U+E040/U+E041을 기본 글자색으로 직접 넣어 검은 괄호가 됐고, 두 번째 교정은 Completion `MacroCode.Fixed`를 넣었으나 해당 전투 대사 UI가 이를 문구로 확장하지 않아 실게임에서 `ケフカ:`와 공백/줄바꿈만 표시됐다. `v2026.08.08.0023` Release와 태그는 회수했다. 현재 후보는 정확한 한국어 키 2개와 원본 값에만 제한하여 `#7FBF5F` 여는 색상 + U+E040 + 색상 복원, 한국어 원문, `#C1584F` 닫는 색상 + U+E041 + 색상 복원 순으로 직접 직렬화하고, 줄바꿈도 raw LF가 아닌 `02 10 01 03` SeString 매크로로 복원한다. 회귀 검사는 공백, ASCII `7`/`8`, `Fixed` payload를 허용하지 않고 생성 row의 119바이트 전체를 대조한다. 실제 전투 화면에서 초록 여는 괄호·빨간 닫는 괄호와 두 한국어 문구가 확인되기 전에는 해결 또는 배포 완료로 간주하지 않는다.
