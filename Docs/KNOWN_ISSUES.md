...
- 2026-05-30 applied-state result: installed `D:\SquareEnix\FINAL FANTASY XIV - A Realm Reborn\game` matches `%LOCALAPPDATA%\FFXIVKoreanPatch\generated-release\ja\2026.05.01.0000.0000\20260530-002104` for generated font/UI outputs. That generated output passes focused lobby runtime checks and `font-runtime-glyph-bounds`, so the remaining crash work must either capture a real native stack for the crashhandler-only termination or add a new verifier that fails this exact applied output before changing font placement.
