...
- 2026-05-30 r36 4K lobby coverage recheck: the old broad `4k-lobby-font-derivations` missing-glyph caveat is not reproduced on current `.tmp\lobby-highscale-ascii-all-ja-r1`. The check passes with `static=144`, `addon-derived=136`, `total=280`; high-scale lobby candidates are visible and do not fall back to `-`/`=`.
- 2026-05-30 r36 release-gate strengthening: the default `verify-hd-crash-release.ps1` focused checks now include the repeated live-regression surfaces (`start-screen-glyph-variants`, `lobby-render-snapshots`, `action-detail-scale-layouts`, `pvp-profile-font-routes`, `combat-flytext-damage-glyphs`, `third-party-game-font-safety`, `party-list-self-marker`) instead of relying on `-RunFullFontObjective` alone. `lobby-critical` and `ingame-critical` groups were expanded with the same current guards plus 4K lobby phrase, reported in-game phrase, source-preservation, and TTMP texture-neighborhood checks. Current `.tmp\lobby-highscale-ascii-all-ja-r1` passes the strengthened default gate with `-SkipGenerate -AppliedGame`, and the expanded `font-critical` group passes in `.tmp\hd-crash-release-gate-logs\manual-font-critical-expanded.log`.
- 2026-05-30 r36 visual snapshot gate option: `verify-hd-crash-release.ps1 -DumpVisualSnapshots` reruns `lobby-render-snapshots` without `-NoGlyphDump` and writes PNG previews under `.tmp\hd-crash-release-gate-logs\*-visual-snapshots`. When `-AppliedGame` is supplied, the snapshots render the applied game files, not just the generated output folder. Use this when reporting lobby/system-settings progress so the same gate produces inspectable `시스템 설정`, character-select, FHD, and high-scale result-message renders. Verified with a local hardlink applied-game mirror; `20260530-230341-visual-snapshots` contains applied-state PNGs.
- 2026-05-30 current installed-state note: the real installed `D:\SquareEnix\FINAL FANTASY XIV - A Realm Reborn\game` no longer matches `.tmp\lobby-highscale-ascii-all-ja-r1` for many `000000` font files, although the checked `060000` UI files still match. Do not use that installed folder as current-output evidence until the current generated output is applied again and `applied-output-files` passes.
...
- Strong release gate also passed without `-SkipKnownFailureCheck`: `verify-hd-crash-release.ps1 -SkipGenerate -RunFullFontObjective -FailOnAnyNewCrash` rejected the historical crash/font outputs `20260528-003930`, `.tmp\release-hd-crash-head-ja`, `20260530-002104`, and `.tmp\start-variant-ja-r3`, then passed the post-release crash-log check.
- Applied-state follow-up: `.tmp\start-variant-visual-alias-ja-r3` was applied to `D:\SquareEnix\FINAL FANTASY XIV - A Realm Reborn\game` after confirming no FFXIV/XIVLauncher processes were running. Backup was written to `.tmp\applied-route-backups\20260531-013525`; `verify-hd-crash-release.ps1 -SkipGenerate -AppliedGame ... -SkipKnownFailureCheck -SkipCrashLogCheck` then passed `applied-output-files`, `applied-lobby-routes`, `lobby-runtime-font-safety`, and `font-runtime-glyph-bounds`.
- Applied full visual gate: `verify-hd-crash-release.ps1 -SkipGenerate -AppliedGame ... -RunFullFontObjective -DumpVisualSnapshots -FailOnAnyNewCrash` passed against the installed game folder. Snapshot PNGs are under `.tmp\hd-crash-release-gate-logs\20260531-015441-visual-snapshots`; inspected samples include lobby settings 150/200/300% result text, character-select `TrumpGothic_23/34_lobby`, ActionDetail `즉시 발동`/`120.00초`/`1.50초`, and PvP `크리스탈라인 컨플릭트`.
- Applied objective gate hardening: `verify-hd-crash-release.ps1 -RunFullFontObjective -AppliedGame` now passes `--applied-game` plus clean font/UI indexes into every objective chunk, instead of relying on generated-output chunks after `applied-output-files`. Re-run passed with logs `20260531-022850-objective-lobby-critical.log` through `20260531-024229-objective-texture-neighborhoods.log`.
- Weak applied-state gate: `-SkipFocusedChecks` was added for fast post-apply drift checks. The quick command with `-SkipGenerate -SkipFocusedChecks -AppliedGame ... -SkipKnownFailureCheck -SkipCrashLogCheck` passed `applied-output-files`, `applied-lobby-routes`, `lobby-runtime-font-safety`, and `font-runtime-glyph-bounds` in the installed game folder; it is not a substitute for the full objective/visual gate.
- Completion-audit note: use only the completed `20260531-022850` through `20260531-024229` applied objective logs as current full-objective evidence. The later `20260531-171804-objective-lobby-critical.log` was interrupted and is not valid completion evidence.
