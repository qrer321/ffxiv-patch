...
- 2026-05-23 PvP correction: `PvpProfileVisualScaleGlyphs.TargetFontPaths` is intentionally empty. The earlier Jupiter visual-scale repair made live PvP profile labels thin/small; the current guard preserves TTMP source scale for `Jupiter_16.fdt` and `Jupiter_20.fdt` and keeps PvP ULD font/render bytes clean. `pvp-profile-font-routes` still emits advisory CJK/digit ratio warnings, so the live PvP visual issue remains pending user confirmation.
- 2026-07-31 shared UI correction: the selective PvP repair had been re-enabled for shared `Jupiter_16/20.fdt`, so PvP-derived Hangul was globally reduced while characters outside that set stayed at source size. This reproduced the Free Company `부대 혜택` report with relative glyph-scale spreads `1.385/1.438` and also affected Action/PvP routes. The repair targets are empty again; the generator source-preserves both Jupiter FDT/TEX routes while reserving the retired transform's 236 atlas cells per font to keep later allocations stable. `shared-ui-font-consistency` follows Action, PvP, and Free Company ULDs and checks 51/51 phrase/font probes plus 102/102 Jupiter source glyphs. Fresh output `.tmp\shared-ui-font-consistency-ja-20260731` passes the focused regression set, but live 100% UI-scale confirmation is still required.
- 2026-07-31 100% scale follow-up: live testing confirmed that the mixed-size glyph defect is gone and that 150%+ remains correctly sized, but source-preserved `Jupiter_16/20.fdt` is uniformly oversized at 100% (`1.58..1.93x` the same-font digit height). The 100% routes now repoint every visible Hangul entry, not a phrase subset, to already-loaded lower-scale cells (`Jupiter_16 <- AXIS_14`, `Jupiter_20 <- Jupiter_16`); invisible composition fillers U+115F/U+1160 retain their original routes. This changes no ULD bytes, texture pixels, texture pages, or 150%+ `Jupiter_23/46` and `TrumpGothic_34/68` routes. Fresh output `.tmp/shared-ui-100pct-low-scale-ja-20260731-v2` passes the 12-check regression suite with 22,700 mapped routes and 0 failures. Live testing accepted the consistency fix, with a remaining subjective note that 100% still looks slightly large.
- 2026-08-01 all-font scale audit: `ingame-font-scale-survey` now compares all 11,172 modern Hangul syllable metrics in each measurable non-lobby FDT, samples rendered height, and records the FDT `Size`, `LineHeight`, and `Ascent`. It correctly flags the pre-fix `Jupiter_16/20.fdt` output at `visible height / Size = 1.120`, while the current `.tmp/shared-ui-100pct-low-scale-ja-20260731-v2` has 0 oversized-font candidates. `Jupiter_23.fdt` is not another low-scale defect: it is source-identical, measures `22.406 / 23 = 0.974`, and remains inside its 35px line box; its high Hangul/digit ratio comes from the short Jupiter digits.
- 2026-08-01 partial-scale audit caveat: `TrumpGothic_34/68.fdt` intentionally change 898 of 11,172 modern Hangul syllables. The changed/unchanged rendered-height spreads are `1.182` and `2.089`, respectively. The 898-codepoint set covers every Hangul syllable currently collected from `Addon`, `ContentFinderCondition`, `ContentRoulette`, and `ContentType`, and the focused ActionDetail/PvP/lobby regression checks pass, so this is not a confirmed live defect. It remains a latent mixed-size risk if a future or dynamic string reaches a `TrumpGothic_23` title node with Hangul outside that set; do not expand or replace the shared route without a concrete ULD/text mapping and atlas-capacity proof.
- 2026-08-01 Party Bonus 100% correction: `ui/uld/PartyMemberList.uld` uses `FontType=3`, `FontSize=19`, which resolves to the previously uncorrected `TrumpGothic_184.fdt` only at 100%; 150/200% resolve to `TrumpGothic_34.fdt` and 300% to `TrumpGothic_68.fdt`. The generator now applies a low-scale `1.03` Hangul-to-digit target to the same 898-codepoint visual-scale set without changing ULD bytes or upper-scale routes. The reported labels render at advances `83/121/121` inside their 122px slot spacing. Fresh output `.tmp/party-bonus-low-scale-ja-20260801-r2` passes the 14-check regression suite; the scale-route verifier covers 371 text nodes across 22 ULDs and all 3,592 routed TrumpGothic glyph transformations. This result is generated-output verification only; live 100/150/200/300% client confirmation remains pending.
- 2026-08-01 Party Bonus verifier correction: the r2 result above was rejected after live testing. Its verifier compared text against the 122px component stride, but the left-aligned text node starts at x=24 and therefore has only 98px available; it also omitted the `Defense`/`Healing` Addon rows 1619-1620. The replacement patches only component 1020/node 4 in `ui/uld/PartyMemberList.uld` from `TrumpGothic 19` to `AXIS 12`, fails closed unless the complete ULD structure matches, and verifies that exactly the two font bytes at `0x3886/0x3887` changed. The longest labels measure `93/135/180/261px` against available widths `98/147/196/294px` at 100/150/200/300%. Fresh output `.tmp/party-bonus-route-font-ja-20260801-r3` passes all 14 regression checks; live confirmation of this replacement remains pending.
- 2026-08-01 exhaustive font-size audit correction: `AXIS_96.fdt`, used by five 100% `ContentsFinder.uld` nodes, was missing from the non-lobby font survey. It is now covered and is source-identical (`9.313px` visible Hangul height for nominal size `9.6`). Of 27 packaged non-lobby FDTs, 22 contain measurable modern Hangul and five are numeric/display fonts without modern Hangul. The only partial-size fonts are `TrumpGothic_23/34/68.fdt`: each transforms 898 of 11,172 Hangul syllables, with transformed/untransformed mean-height spreads `1.084/1.182/2.089`. Their scale route is shared by 21 text nodes across Action, Social/Party, Free Company, Item Detail, Character, Content Finder, HUD Layout, Teleport, and PvP ULDs. The selected coverage includes every Hangul syllable currently found in `Addon`, `MainCommand`, `ContentFinderCondition`, `ContentRoulette`, and `ContentType`, so this is a latent dynamic/unmapped-string risk rather than another confirmed live defect. The survey threshold is now 5%, so all three partial fonts are reported instead of hiding `TrumpGothic_23` below the old 15% threshold. Lobby large-label, scale-source, and render-snapshot checks still pass separately.
- 2026-08-02 generated-output coverage correction: the 898-codepoint allowlist above was too narrow for the shared non-lobby route. An automatic scan of 1,037 readable ULDs found 12,407 text nodes, including 827 `FontType=3`/size-23 nodes across 677 ULDs, so Action/PvP/Free Company/Party Bonus names are not a sufficient boundary. The generator now collects Hangul from every EXD page it actually writes and transforms the resulting 1,891 Hangul/Jamo codepoints in all three shared scale tiers, `TrumpGothic_23/34/68.fdt`. Scaled alpha keeps the prior visual result, transparent top/right/bottom padding is trimmed, and larger cells are allocated first; any atlas-capacity miss aborts before FDT/texture commit. Fresh `.tmp/shared-ui-patched-output-coverage-ja-20260802-r7` passes the 14-check regression suite and reports 0 oversized-font candidates. Arbitrary player-entered or future rare Hangul outside the generated EXD set remains a residual mixed-scale risk: preallocating all 11,172 syllables does not fit the current atlas and must continue to fail closed rather than expand texture dimensions or reuse unsafe cells.
- 2026-08-02 clean-baseline verifier correction: an intermediate raw SqPack scan incorrectly counted five Hangul syllables from the already-patched global installation when an output page still referenced DAT0. Generated-output coverage must resolve unchanged entries through the sibling `orig.0a0000.win32.index`, not the active game index. The corrected composite scan derives 993 additional output codepoints and 1,891 total; `ingame-font-scale-routes` verifies all 1,891 glyphs in each shared tier, while `third-party-game-font-safety` confirms 154,240 untouched Hangul routes/neighborhoods remain source-identical.
- 2026-08-02 Duty Finder role-tab correction: `ContentsFinder.uld` widget 1/node 24 renders `Addon#2503` `역할` in a 30x26 slot, while node 28 receives `Addon#2784-2786` `방어 역할`/`회복 역할`/`공격 역할` in a 70x26 slot. Both used `TrumpGothic 23`; the corrected shared glyphs still measured 44px and 96px at 100%, and the same proportional overflow occurred at 150/200/300%. The generator now changes only these two nodes to `AXIS 12` after validating their widget/node identity, rectangle, TextId, alignment, flags, spacing, and source font. The verifier requires exactly four changed font bytes at `0x10656/0x10657/0x107E6/0x107E7`, follows the `AXIS_12/18/18/36` scale routes, and checks all four phrases against their scaled slots. Fresh `.tmp/shared-ui-patched-output-coverage-ja-20260802-r8` measures 23px/53-54px at 100% and passes the full 14-check regression suite.
- 2026-08-05 High-end Duty Finder correction: 고난도 임무 찾기는 일반 임무 찾기의 `ContentsFinder.uld`가 아니라 구조가 거의 같은 `RaidFinder.uld`를 사용하므로 기존 역할 탭 보정이 적용되지 않았다. 제너레이터는 이제 두 ULD의 widget 1/node 24·28 계약을 각각 검증하고 각 파일에서 해당 폰트 4바이트만 `TrumpGothic 23`에서 `AXIS 12`로 변경한다. 직업명 node 32는 두 ULD 모두 기존 `Jupiter 23`, 180x34 영역을 유지하며, `건브레이커`/`암흑기사`/`픽토맨서`가 100/150/200/300% 배율에서 영역 안에 들어오는 것을 검증한다. `.tmp/shared-ui-patched-output-coverage-ja-20260805-r9`은 일반/고난도 역할명 및 직업명 배율 검증과 공유 UI 회귀 검사를 통과했고, 사용자가 실제 고난도 임무 화면에서 임무명이 정상 표시되는 것을 확인했다. 같은 변경에서 선택형 `임무명` 원문 유지 옵션을 추가했으며, 이 옵션은 `ContentFinderCondition`의 `Name`(offset 0)과 `NameShort`(offset 4)만 선택한 일본어/영어 베이스 언어로 유지하고 기본값은 기존과 동일하게 한국어 번역이다.
- 2026-08-08 케프카 전투 시작 대사 재교정: 최초 교정에서 자동번역 표시용 U+E040/U+E041 문자를 EXD에 직접 넣었으나, 실게임에서는 자동번역 괄호가 아니라 검은 화살표 글리프로 표시되어 해당 릴리즈를 즉시 비공개·태그 삭제했다. 원인은 외부 `rsv.json`의 `InstanceContentTextData#45500` 한국어 값에서 두 `MacroCode.Fixed` 페이로드가 ASCII `7`/`8`로 평탄화된 것이다. 제너레이터는 이제 정확한 `_rsv_45500_-1_6_*_S13095D61_E13095D61` 키와 원본 값만 받아 Completion group/key `2/24`(`여기는 처음 옵니다.`), `2/27`(`잘 부탁합니다!`)을 가리키는 실제 fixed-macro 바이트 `02 2E 03 02 19 03`/`02 2E 03 02 1C 03`으로 복원하며, 입력이 달라지면 생성을 중단한다. 회귀 검사는 생성 archive의 raw EXD 바이트와 Completion row 224/227을 함께 검사한다. 잘못된 PUA 산출물에서 실패를 재현했고, `.tmp/kefka-fixedmacro-patched-output-ja-20260808`은 새 검사를 포함한 15개 전체 회귀 검사를 통과했다. 실게임 최종 확인 전까지 해결 완료로 간주하지 않는다.
