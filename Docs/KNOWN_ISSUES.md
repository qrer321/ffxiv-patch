...
- 2026-05-20 r20 lobby large-label no-shrink: the user confirmed the UI-scale crash is gone, then reported that 로비 `시스템 설정` and character-select labels such as `종족 및 성별` still look small. The previous verifier allowed `TrumpGothic_23_lobby/34_lobby` Hangul at about `0.88x` of the `AXIS_18_lobby` source route. The generator now refuses sub-1.0 visual shrink for these lobby large-label fonts and reuses the canonical source glyph cell when no actual scale transform is applied, so six-page capacity is not consumed by duplicate cells. `.tmp\lobby-large-label-noshrink-ja-r3` passes the focused lobby coverage/large-label/in-game safety set.
- 2026-05-20 PvP profile follow-up: `크리스탈라인 컨플릭트`-related PvP profile font still needs a separate route-scoped pass. Current verifier output shows `pvp-profile-font-routes` passing but retaining advisory size warnings on routed `AXIS_12/14/18.fdt` labels. Do not solve this by globally resizing AXIS glyphs; any fix must be ULD-route scoped or otherwise prove `third-party-game-font-safety`, `action-detail-scale-layouts`, and damage fly text remain clean.
- 2026-05-20 PvP ULD route correction: current code generates `.tmp\pvp-profile-uld-ja-r1`, which patches only PvP profile/action/team ULD text-node routes from `AXIS_12/14/18` to existing PvP `Jupiter_16/20` routes. The verifier now treats unexpected PvP ULD render-byte changes as failures and confirms zero remaining PvP `AXIS_*` ULD routes. Keep this as code-level verification until user confirmation.
