...
- 2026-08-07 케프카 전투 시작 대사 교정: `InstanceContentTextData#45500`의 외부 RSV 값은 자동번역 여닫기 글리프를 ASCII `7`/`8`로 잘못 담고 있어 한국어 대사 양옆에 숫자가 그대로 표시됐다. 제너레이터는 정확히 `_rsv_45500_*_S13095D61_E13095D61` 토큰 계열만 U+E040/U+E041로 복원하고, 구분자 순서가 바뀌거나 짝이 맞지 않으면 생성을 중단한다. 기존 산출물에서 새 `rsv-auto-translate-delimiters` 검사가 실패하는 것을 재현했고, 수정 산출물의 실제 EXD row에서 두 한국어 문구의 글리프 복원을 확인했다. `.tmp/kefka-autotranslate-full-ja-20260807`은 새 검사를 포함한 15개 전체 회귀 검사를 통과했으며, `BattleTalk.uld` 본문 경로의 `AXIS_18.fdt` U+E040/U+E041 글리프와 텍스처 이웃도 글로벌 원본과 동일하다.
