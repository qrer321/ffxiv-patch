...
- 요청일: 2026-05-22
- 구현 예정 상태입니다. 현재 패치 동작을 바꾸지 않습니다.
- 후보 프로필:
  - `common-base`: 상용구/시스템성 공통 문구는 선택한 베이스 클라이언트 언어로 유지하고, 나머지 패치는 기존 전체 한글 패치 정책을 따릅니다.
  - `story-only`: UI, 시스템, 전투, 일반 게임플레이 문구는 베이스 클라이언트 언어로 유지하고, 스토리/퀘스트/대화 계열만 한국어로 패치합니다.
- 구현 시 특정 문구를 하드코딩하지 말고 sheet/row/column 정책 그룹으로 보존 범위를 정의해야 합니다.
- 구현 전 검증 조건:
  - 보존 대상 row는 선택 언어의 clean/global 원문과 일치해야 합니다.
  - `story-only`에서는 비스토리 sheet가 베이스 언어로 남고, 스토리 sheet만 한국어 매핑을 사용해야 합니다.
  - SeString 제어 코드, 아이콘, 링크, 매크로 토큰은 패치 전후 동일해야 합니다.
  - 베이스 클라이언트 언어별 `ja/en` 결과가 각각 분리 검증되어야 합니다.
...
- 2026-05-22 보강: 저배율/작은 채팅창/인스턴스 표기에서 번호 glyph 주변 픽셀이 섞이지 않도록 `U+E031`, `U+E037`, `U+E0B1`~`U+E0B8`, `U+E0E1`~`U+E0E8`은 dirty target cell을 재사용하지 않고 8px base/mip 주변 영역까지 clean PUA cell로 검증한다.
...
- 릴리즈 기본값에서는 TTMP의 FDT/texture 조합을 한 세트로 유지. 예외적으로 파티 리스트 본인 번호 및 인스턴스 번호 PUA glyph(`U+E0B1`~`U+E0B8`, `U+E0E1`~`U+E0E8`)는 clean global 모양으로 복원하며, TTMP atlas의 빈 영역을 찾아 해당 glyph 픽셀과 FDT 엔트리를 함께 보정
