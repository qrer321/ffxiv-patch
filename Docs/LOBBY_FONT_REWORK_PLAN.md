...
- 2026-05-27 startup small-font texture-slot guard: HD/non-4K crash logs show the startup/lobby small-font renderer can dereference a null buffer when Hangul uses texture slot 4 (`font_lobby5.tex`). Keep `AXIS_12/14/18_lobby` and `MiedingerMid_12/14/18_lobby` allocations inside `font_lobby1..4` (`image_index < 16`). Do not apply this limit globally, because `TrumpGothic_*_lobby` large labels and high-scale lobby routes still need the clean six-page capacity to avoid coverage/scale regressions. `lobby-runtime-font-safety` must fail old outputs that put startup-limited small-font Hangul on `font_lobby5/6`.
