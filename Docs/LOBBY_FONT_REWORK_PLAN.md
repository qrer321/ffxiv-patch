...
- 2026-05-18 strict capacity rule: `lobby-atlas-capacity` is report-only, while `lobby-full-coverage-capacity` is a hard gate for broad/full lobby coverage. On `.tmp\lobby-large-label-expanded-ja-r2`, the strict gate fails with `target_fonts=18`, `required=16200`, `missing_target=1386`, `source_covered=1386`, and `aggregate_allocation_failures=1081`. Do not keep shrinking coverage until it fits; that recreates the user's unreported `-`/`=` problem. The next implementation must design a texture-page split or a complete lobby font set, then make this strict verifier pass with `aggregate_allocation_failures=0`.
- 2026-05-18 page-split sizing result: `lobby-page-split-capacity` shows full coverage fits with one additional synthetic 1024x1024 page. The global index currently has `font_lobby1.tex` through `font_lobby6.tex`; `font_lobby7.tex` is not present by default. Next implementation must handle the index/index2 addition or another proven page route before writing FDT `image_index >= 24`.
- 2026-05-19 implementation result: the generator can now create an additional blank `common/font/font_lobby7.tex` from a clean lobby texture template, add it to both `000000.win32.index` and `000000.win32.index2`, and route lobby FDT glyphs to it. Fresh output `.tmp\lobby-page7-ja-r2` passes `lobby-full-coverage-capacity,lobby-page-split-capacity,lobby-multitexture-font-set,lobby-coverage-glyphs` with `target_fonts=18`, `required=16200`, `missing_target=0`, `aggregate_allocation_failures=0`, and seven referenced lobby textures. This is code-level/generated-output verification only; do not call the live client result fixed until the user confirms it.
- 2026-05-19 live rejection: after applying the page7 release, UI-scale changes crashed. The applied client payload matched the generated output, so the failure is not an installer drift issue. Do not use added lobby texture pages or lobby `image_index >= 24`; the client runtime scale reload appears limited to the clean six lobby pages. The verifier now treats clean-missing lobby texture pages as failures, and the generator must fall back to a crash-safe six-page route even if full-sheet capacity no longer fits.
- 2026-05-19 crash-safe output: `.tmp\lobby-no-page7-ja-r1` is the current safe baseline for release because it removes page7 allocation and passes the focused lobby/in-game safety group. It intentionally does not solve full lobby coverage; the next coverage design must avoid both atlas resizing and added lobby texture pages, or prove a complete replacement font set that the client reload path accepts.
- 2026-05-20 six-page correction: full lobby coverage can fit without page7 after fixing allocator waste. The kept design uses only clean `font_lobby1.tex` through `font_lobby6.tex`, does not resize textures, and uses no added clean-missing lobby page. Dense allocation has a per-size/channel cursor so it can use fragmented clean-page holes without rescanning the whole atlas for every glyph. Compatible visual-scale large-label fonts share identical Hangul cells instead of allocating duplicates. Fresh output `.tmp\lobby-dense-sixpage-ja-r3` passes `lobby-full-coverage-capacity,lobby-coverage-glyphs,lobby-multitexture-font-set,lobby-large-label-scale-layouts,lobby-scale-font-sources,start-system-settings-uld,start-main-menu-phrase-layouts,third-party-game-font-safety,combat-flytext-damage-glyphs,ingame-clean-ascii-glyphs`.
- 2026-05-20 large-label no-shrink correction: the crash-safe six-page output still made lobby/character-select large labels look small because `TrumpGothic_23_lobby/34_lobby` Hangul was scaled down to about `0.88x` of the `AXIS_18_lobby` source glyph route. The corrected rule is: if the visual scale would be below `1.0`, do not shrink; if the resulting scale is effectively `1.0`, reuse the canonical `AXIS_18_lobby` allocation key and texture cell instead of making a duplicate visual-scale allocation. This keeps full coverage inside six clean lobby pages. Fresh output `.tmp\lobby-large-label-noshrink-ja-r3` passes `lobby-large-label-scale-layouts,lobby-multitexture-font-set,lobby-coverage-glyphs,lobby-full-coverage-capacity,third-party-game-font-safety,combat-flytext-damage-glyphs`.
- 2026-05-20 subset-scale correction: forcing `1.08x` on all 907 `TrumpGothic_23_lobby/34_lobby` glyphs makes high-scale lobby fonts miss 86 glyphs each, so broad visual scaling is rejected. Keep full 907-glyph coverage unscaled, and apply `1.08x` only to the large-label subset used by start system settings and character-select info labels. Fresh local-origin output `.tmp\lobby-large-label-min108-subset-ja-r1` passes `lobby-large-label-scale-layouts,lobby-scale-font-sources,lobby-full-coverage-capacity,pvp-profile-font-routes,third-party-game-font-safety,combat-flytext-damage-glyphs,action-detail-scale-layouts`.
- 2026-05-23 source-route correction: `TrumpGothic_34_lobby` must not be treated as another `AXIS_18_lobby` visual-scale copy. Route survey shows start-system settings uses `TrumpGothic_23_lobby`, while character-select large labels can use `TrumpGothic_34_lobby`; therefore 34_lobby uses `TrumpGothic_34.fdt` as the visual source and only the character-label subset. Fresh output `.tmp\lobby-runtime-scale-source-ja-r5` passes full coverage, six-page texture routing, lobby large-label scale, lobby source routes, start-system-settings ULD, PvP route, and the shared in-game regression set. Remaining caveat: 4 fallback allocations per high-scale lobby font are tolerated only as a crash-safe six-page fallback, not as a live-confirmed visual fix.
- 2026-05-23 UI-resolution crash guard: non-4K UI resolution boot crashes must be tested separately from UI scale. Do not solve this by adding `font_lobby7.tex`, expanding textures, or shrinking coverage. The current route keeps only clean six lobby pages and adds `lobby-texture-cell-margin`: non-high-scale lobby glyph allocations may not touch the bottom texture edge unless the same glyph already did so in clean. High-scale lobby supplemental fonts are processed first and are exempt from the bottom-edge margin so large 4K source glyphs do not fall back to smaller cells. Fresh output `.tmp\lobby-uires-safe-ja-r6` passes the margin, coverage, capacity, scale-source, large-label, start-system-settings, PvP, damage fly-text, party marker, clean ASCII, and third-party shared-font guards. Live confirmation is still required before closing the crash report.
- 2026-05-27 startup small-font texture-slot guard: HD/non-4K crash logs show the startup/lobby small-font renderer can dereference a null buffer when Hangul uses later lobby texture slots. The first rejected output pointed at slot 4 (`font_lobby5.tex`), and the later rejected output `20260527-225850` pointed at slot 3 (`font_lobby4.tex`) through `common/font/AXIS_12_lobby.fdt image_index=12/13`. Keep `AXIS_12/14/18_lobby` and `MiedingerMid_12/14/18_lobby` allocations inside `font_lobby1..3` (`image_index < 12`). Do not apply this limit globally, because `TrumpGothic_*_lobby` large labels and high-scale lobby routes still need the clean six-page capacity to avoid coverage/scale regressions. Use startup-safe shared clean CJK reclaim cells for the small-font group rather than shrinking sheet coverage.
- 2026-05-23 r25 route-scoped clean-page reclaim: the bottom-edge exemption above is no longer allowed. All lobby glyph allocations, including high-scale routes, must keep a 1px bottom margin unless clean already owned the route. To recover capacity without added pages, reclaim only clean Japanese/CJK cells inside the target FDT's own clean page set and never from clean-unused pages. The current generated output `.tmp\lobby-route-scoped-ja-r1` passes focused lobby checks for the observed active routes and confirmed in-game regressions. Do not expand this by re-adding unobserved `Jupiter_46_lobby`, `Jupiter_90_lobby`, `Meidinger_40_lobby`, `MiedingerMid_36_lobby`, or `TrumpGothic_68_lobby` scale routes until a ULD route survey proves they are active and a clean-page-compatible allocator passes.
- 2026-05-19 regression note: `.tmp\lobby-page7-ja-r2` also passes the scoped lobby group (`lobby-large-label-scale-layouts,lobby-scale-font-sources,start-system-settings-uld,start-main-menu-phrase-layouts`) and in-game safety group (`third-party-game-font-safety,combat-flytext-damage-glyphs,ingame-clean-ascii-glyphs`). The broader `action-detail-scale-layouts,pvp-profile-font-routes` group still fails on the known `common/font/Jupiter_23.fdt` large UI TTMP-source mismatch and must be handled as the next font-scale task, not hidden by the lobby page split.
