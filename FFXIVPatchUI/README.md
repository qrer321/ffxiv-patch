...
실행 파일 옆 `Release\Public` 또는 `Release\Test` 폴더는 배포 산출물 위치입니다. 백업/복구 기준 폴더는 이 배포 산출물 폴더와 분리되어 있어 빌드 산출물을 다시 만들어도 삭제되지 않습니다.
...
4. UI가 `FFXIVPatchGenerator.exe`를 실행합니다.
5. 제너레이터 진행도는 UI progress bar에 표시됩니다.
6. 생성된 release 파일의 `manifest.json`을 기록합니다.
7. 적용 대상 파일을 먼저 백업합니다.
8. index/index2/dat1 파일을 글로벌 클라이언트 sqpack 폴더에 복사합니다.
9. 작업 결과와 로그 경로를 결과 창에 표시합니다.

## 패치 제거와 복구

`한글 패치 제거`는 패치 적용 시 생성된 `orig.*.index/index2` 또는 로컬 복구 기준을 찾아 index를 원래 상태로 되돌립니다. `dat1` 파일이 남아 있어도 index가 참조하지 않으면 게임은 해당 패치 데이터를 사용하지 않습니다.
